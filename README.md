...
Bourne provides a number of options for flow control. In the example above, we've defined a composition of 
...
Bourne provides the capability to importing additional configuration entities. This is accomplished via the `import` directive.
```xml
<import path="./app-cli/src/test/resources/global_settings.xml" />
```
...
<story id="global_settings">
    ...
    <archives>
        <FileArchive id="DataStore" base="{{ user.home }}/bourne/archive" compression="gzip" />
    </archives>
...
Bourne provides the capability of loading predefined configuration properties from a file. These properties could be details that
...
Bourne also provides many data ingestion options, including file-monitoring capabilities. The following is an example of a file monitoring 
agent (FileTrigger) watching a path (e.g. "{{ user.home }}/bourne/incoming/tradingHistory") for four distinct file patterns 
...
<FileTrigger id="trading_history_trigger">
    <directory path="{{ user.home }}/bourne/incoming/tradingHistory" archive="DataStore">
...
Bourne provides a mechanism for archiving files. This is normally used in conjunction with a `FileTrigger` directive. Simply put
...
Bourne uses the concept of a `layout` to define the input or output format of a data source. Data formats like CSV, JSON, Avro and others
...
Currently Bourne offers a single input source type; however, more will be added soon, including Kafka, RDBMS and others.
...
Bourne provides a number of options for data persistence, including Azure DocumentDB, Kafka, MongoDB, RDBMS, and Text Files (delimited, CSV, Fixed-length, JSON and soon XML).
